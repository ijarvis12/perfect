...
static int lcf_main (lua_State * L) {
  ...
  lua_getfield(L,LUA_ENVIRONINDEX,"print");
  lua_pushliteral(L," This program finds perfect numbers using Mersenne Primes ");
  lua_call(L,1,0);
  assert(lua_gettop(L) - lc_nextra == 0);
  ...
  lua_getfield(L,LUA_ENVIRONINDEX,"print");
  lua_pushliteral(L,"Enter max exponent #: ");
  lua_call(L,1,0);
  assert(lua_gettop(L) - lc_nextra == 0);
  ...
  while ((((lc12_step > 0) && (lc10_var <= lc11_limit)) || ((lc12_step <= 0) && (lc10_var >= lc11_limit)))) {
    ...
    /* internal: local p at index 1 */
    lua_pushnumber(L,lc10_var);
    
    /* psum = 0 */
    lua_pushnumber(L,0);
    lua_setfield(L,LUA_ENVIRONINDEX,"psum");
    assert(lua_gettop(L) - lc_nextra == 1);
    
    /* --[[   the potential perfect number --]]
     * perfect = 2^(p)*(2^(p+1)-1) */
    lua_pushnumber(L,2);
    lc_pow(L,-1,(1 + lc_nextra));
    lua_remove(L,-2);
    lua_pushnumber(L,2);
    lua_pushnumber(L,1);
    lc_add(L,(1 + lc_nextra),-1);
    lua_remove(L,-2);
    lc_pow(L,-2,-1);
    lua_remove(L,-2);
    lua_remove(L,-2);
    ...
    lua_remove(L,-2);
    lua_remove(L,-2);
    lua_setfield(L,LUA_ENVIRONINDEX,"perfect");
    assert(lua_gettop(L) - lc_nextra == 1);
    ...
    lua_gettable(L,-2);
    lua_remove(L,-2);
    lua_getfield(L,LUA_ENVIRONINDEX,"perfect");
    lua_call(L,1,LUA_MULTRET);
    lua_call(L,(lua_gettop(L) - lc14),1);
    lua_setfield(L,LUA_ENVIRONINDEX,"sqrtp");
    assert(lua_gettop(L) - lc_nextra == 1);
    
    /* --[[   add up all the divisors into psum --]]
     * for n = 1,sqrtp,1 do */
    lua_pushnumber(L,1);
    lua_getfield(L,LUA_ENVIRONINDEX,"sqrtp");
    lua_pushnumber(L,1);
    if (!(((lua_isnumber(L,-3) && lua_isnumber(L,-2)) && lua_isnumber(L,-1)))) {
      luaL_error(L,"'for' limit must be a number");
    }
    ...
    while ((((lc17_step > 0) && (lc15_var <= lc16_limit)) || ((lc17_step <= 0) && (lc15_var >= lc16_limit)))) {
      ...
      /* internal: local n at index 2 */
      lua_pushnumber(L,lc15_var);
      
      /* if perfect % n == 0 then */
      enum { lc19 = 2 };
      lua_getfield(L,LUA_ENVIRONINDEX,"perfect");
      lc_mod(L,-1,(2 + lc_nextra));
      lua_remove(L,-2);
      lua_pushnumber(L,0);
      ...
      if (lc21) {
        ...
        /* psum = psum + n */
        lua_getfield(L,LUA_ENVIRONINDEX,"psum");
        lc_add(L,-1,(2 + lc_nextra));
        lua_remove(L,-2);
        lua_setfield(L,LUA_ENVIRONINDEX,"psum");
